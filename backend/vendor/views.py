...
class ProductUpdateAPIView(generics.RetrieveUpdateAPIView):
    ...
    def get_object(self):
        vendor_id = self.kwargs['vendor_id']
        product_pid = self.kwargs['product_id']
...
def MonthlyEarningTracker(request, vendor_id):
    ...
    monthly_earning_tracker = (
        ...
        .aggregate(month=ExtractMonth('date'))\
        .values('month')\
        .annotate(
...
class CouponCreateAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        vendor = Vendor.objects.get(id=vendor_id)
        coupon = Coupon.objects.create(vendor=vendor, code=code, discount=discount, active=(active.lower() == 'true'))
...
class CouponStats(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return [{
            'total_counpon': total_coupons,
            'active_coupon': active_coupons
        }]
...
class NotificationSummaryAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        un_read_noti = Notification.objects.filter(vendor=vendor, seen=False).order_by('seen')
        read_noti = Notification.objects.filter(vendor=vendor, seen=True).order_by('seen')
...
class Earing(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        ).aggregate(total_revenue=models.Sum(models.F('sub_total') + models.F('shipping_amount')))['total_revenue'] or 0
