...
function AddProduct() {
  ...
  const handleImageChange = (index, event, setStateFunction) => {
    ...
    if (file) {
      ...
      // Handle case when no files is selected
      setStateFunction((prevState) => {
        const newState = [...prevState];
  ...
  const handleSubmit = async (e) => {
    ...
    if (product.title == "" 
        ...
        // If any required field is missing, show an error message or take approriate action
        setIsLoading(false);
        Swal.fire({
          icon: 'warning',
        ...
        colors.forEach((color, index) => {
          Object.entries(color).forEach(([key, value]) => {
            if(key === 'image' && value && value.file && value.file.type.startsWith('image/')) {
              formData.append(`color[${index}][${key}]`, value.file, value.file.name);
            } else {
              formData.append(`color[${index}][${key}]`, String(value));
            }
          })
        })
